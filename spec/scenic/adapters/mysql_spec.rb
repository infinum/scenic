require "spec_helper"

module Scenic
  module Adapters
    describe Mysql2, :db do
      describe "create_view" do
        it "successfully creates a view" do
          Mysql2.create_view("greetings", "SELECT text 'hi' AS greeting")

          expect(Mysql2.views.map(&:name)).to include("greetings")
        end
      end

      describe "drop_view" do
        it "successfully drops a view" do
          Mysql2.create_view("greetings", "SELECT text 'hi' AS greeting")

          Mysql2.drop_view("greetings")

          expect(Mysql2.views.map(&:name)).not_to include("greetings")
        end
      end

      describe "views" do
        it "finds views and builds Scenic::View objects" do
          ActiveRecord::Base.connection.execute "CREATE VIEW greetings AS SELECT text 'hi' AS greeting"

          expect(Mysql2.views).to eq([
            View.new(
              "viewname" => "greetings",
              "definition" => "SELECT 'hi'::text AS greeting;",
            ),
          ])
        end
      end
    end
  end
end
